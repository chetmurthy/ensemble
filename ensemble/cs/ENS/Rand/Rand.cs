...
public class Rand
{
	...
	static void HandleMsg(Message msg) {
		...
		case UpType.VIEW:
			...
			Console.WriteLine("  coord= " + view.coord);
			Console.WriteLine("  ltime= " + view.ltime);
			Console.WriteLine("  primary= " + view.primary);
			Console.WriteLine("  groupd= " + view.groupd);
			Console.WriteLine("  xfer_view= " + view.xfer_view);
			Console.WriteLine("  params= " + view.parameters);
			Console.WriteLine("  uptime= " + view.uptime);
			Console.WriteLine("  view= " + view.view);
			...
			Console.WriteLine("  am_coord= " + view.am_coord);
		...
		case UpType.HEARTBEAT:
			Console.WriteLine("HEARTBEAT " + msg.time);
			break;
		}
	}
	...
	static public void RandAction (int i) 
	{
		...
		if (p < 6) {
		    ...
		    Console.WriteLine("id=" + my_rank + " Prompt");
			m.Prompt();
		}  else  if (p < 20) {
			...
			Console.WriteLine("id=" + my_rank + " Properties");
			string vsync = "Gmp:Switch:Sync:Heal:Frag:Suspect:Flow:Slander" ;
			string props = null;
			switch (rand.Next () % 5) {
			...
			default:
				throw new Exception("bad random value, abort");
			}
			m.ChangeProperties(props);
		} else if (p < 40) {
