// $Header: /cvsroot/ensemble/maestro/type/Maestro_Types.C,v 1.1 2003/04/26 13:38:32 orodeh Exp $
...
static double initial_tickcount;

int gettimeofday(timeval *tp, void *dummy)
{
    if (initial_time == 0) {
	initial_tickcount = GetTickCount();
	initial_time = time(NULL);
	tp->tv_sec = (long) initial_time;
	tp->tv_usec = 0;
    } else {
	double now_ms = initial_time + (GetTickCount() - initial_tickcount) * 1e-3;

	tp->tv_sec = (long) now_ms / 1000; 
	tp->tv_usec =  ((long) now_ms % 1000) * 1000;
    }
    return 0;
}
...
void 
Maestro_Base::operator>> (ostream& out) const { 
...
ostream& 
operator << (ostream& out, Maestro_Base& hb) {
    hb >> out;
    return out;
}
...
Maestro_EndpID::operator== (Maestro_EndpID &eid) {
    return (strcmp(endpID.name,eid.endpID.name) == 0);
}
...
Maestro_EndpList::Maestro_EndpList(hot_endpt_t endps[], int nendp,
				   Maestro_ErrorHandler *error_handler) :
    Maestro_Base(error_handler) {
    listSize = nendp;
    if (listSize) {
...
Maestro_IntList::Maestro_IntList(Maestro_ErrorHandler *error_handler) :
    Maestro_IntList_Base(error_handler) {}

Maestro_IntList::Maestro_IntList(int &i, Maestro_ErrorHandler *error_handler) :
...
Maestro_IntList::Maestro_IntList(int *elem, int nelem, 
				 Maestro_ErrorHandler *error_handler) :
    Maestro_IntList_Base(elem, nelem, error_handler) {}


Maestro_ULongList::Maestro_ULongList(Maestro_ErrorHandler *error_handler ) :
...
Maestro_ULongList::Maestro_ULongList(Maestro_ULong *elem, int nelem, 
				     Maestro_ErrorHandler *error_handler ) :
...
Maestro_String::Maestro_String(char *str) {
...
Maestro_String& 
Maestro_String::operator= (char *p) {
    assert(size && s != NULL);
    assert(NULL_STRING[0] == 0);  
    assert(size == strlen(s) + 1);
...
void
Maestro_String::operator >> (ostream &out) const {
    assert(size && s != NULL);
    assert(NULL_STRING[0] == 0);  
    assert(size == strlen(s) + 1);
