 ...
 */
/**************************************************************/
// $Header: /cvsroot/ensemble/maestro/Maestro_ClSv.C,v 1.1.1.1 2003/08/24 06:52:17 nystrom Exp $
...
Maestro_ClSv::grpMemb_AcceptedView_Callback(Maestro_GrpMemb_ViewData &gmView,
    ...
    if (xfer_from_oldest) {
	// Roys new code
//	cout << "Maestro_ClSv::grpMemb_AcceptedView_Callbackgrp, Roy's code" << endl;
	msg >> (int &) vd.stateCounter >> (int&) vd.viewType >> vd.xferServers >> vd.servers;
    }
    else {
//	cout << "Maestro_ClSv::grpMemb_AcceptedView_Callback, old code { " << endl;
...
//	cout << "}" << endl;
	...
	vd.myServerRank = vd.servers.lookup(vd.myEndpID);
    }
    else {
	vd.myServerRank = -1;
    }
...
//	cout << "Original xfer code, Maestro_ClSv::grpMemb_ViewMsg_Callback{" << endl;
	view_msg << servers_nextView << xferServers_nextView 
