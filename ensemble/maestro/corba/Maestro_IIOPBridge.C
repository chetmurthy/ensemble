// $Header: /cvsroot/ensemble/maestro/corba/Maestro_IIOPBridge.C,v 1.1 2003/04/26 13:38:31 orodeh Exp $
// 
...
Maestro_IIOP_Connection::dispatcher(void *arg) {
	...
	FD_ZERO(&read_fdset);
	FD_SET((unsigned)conn->fd(), &read_fdset);
...
Maestro_IIOP_Client::dispatcher_Thread(void *arg) {
	...
	tv.tv_usec = MASTRO_SELECT_TV_USEC;
	FD_ZERO(&read_fdset);
	FD_SET((unsigned)client->serverFd, &read_fdset);
...
Maestro_IIOP_Server::connectionListener_Thread(void *arg) {
	...
	tv.tv_usec = MASTRO_SELECT_TV_USEC;
	FD_ZERO(&read_fdset);
	FD_SET((unsigned)server->listenFd, &read_fdset);
