...
type pub_key = (*Shared.DH.pub_key*) string 
type key = string
...
*)
type inner_msg = 
  | Piece of src * Addr.set * nonce * dst * Addr.set * nonce * pub_key
  | Reply of src * Addr.set * nonce * dst * Addr.set * pub_key * Buf.t
...
let create_nonce t ct = 
  ...
  (ct, hex_of_string (Buf.string_of (Cipher.encrypt t.key true res)))
