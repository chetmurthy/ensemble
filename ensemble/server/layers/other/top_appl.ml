...
let hdlrs s ((ls,vs) as vf) {up_out=up;upnm_out=upnm;dn_out=dn;dnlm_out=dnlm;dnnm_out=dnnm} =
  ...
  let set_blocked blocked =
    ...
    (* Don't allow applications to send messages through a blocked stack *)
    if blocked = B then 
      s.layer_s.handle_action := (function 
	| Control Leave -> 
	    s.leaving <- true ;
	    dnnm (create name ELeave[])
	| a -> 
	    failwith (sprintf "sanity:stack is blocked, tried to send %s" 
	      (string_of_action a)))
  in
  ...
  (* Allow applications using the Ensemble-server to send messages directly *)
  s.layer_s.handle_action := handle_action;
