...
let init s (ls,vs) = 
  let dh_key = match !(s.dh_key) with 
	...
	let param = DH.fromFile len in
	let key = DH.generate_key param in
	s.dh_key := Some key;
	(* DH.show_params dh key;*)
	key
  in
  let pub_key = hex_of_string (DH.string_of_pub_key  (DH.get_pub dh_key)) in
...
let hdlrs s ((ls,vs) as vf) {up_out=up;upnm_out=upnm;dn_out=dn;dnlm_out=dnlm;dnnm_out=dnnm}  =
  ...
  let encrypt key = Cipher.encrypt key true
  and decrypt key = Cipher.encrypt key false in
  ...
  let agreed_key_of_bignum bignum = 
    let material = DH.compute_key s.dhl_key bignum in
  ...
  let verified = function
    | Piece (s_endpt,s_addr,d_nonce, d_endpt, d_addr, s_nonce, pub_key) -> (
	log (fun () -> sprintf "got Piece from %s" (Addr.string_of_set s_addr));
	match s.bckgr with 
	  | Idle -> 
	      log (fun () -> "Idle, computing");
	      let bignum =  DH.pub_key_of_string (hex_to_string pub_key) in 
    ...
    | Reply (s_endpt, s_addr, d_nonce, d_endpt, d_addr, pub_key, key) -> 
  ...
  | EView ->
	...
	match new_vs.key with 
	  ...
	  | Security.Common _ -> 
	      let key = Cipher.decrypt_key (get_cipher vs.key) new_vs.key in
  ...
  | EView ->
	...
	match new_vs.key with 
	  ...
	  | Security.Common key -> 
	    ...
	    | Security.Common _ -> 
		let enc_key_sug = Cipher.encrypt_key (get_cipher vs.key) s.key_sug in
