...
module Iov = struct 
  ...
  let t_of_string pool x ofs len = String.sub x ofs len
    
  let len iov = String.length iov

  let empty = ""

  let try_alloc size = String.create size

  let alloc_async pool size cont_fun = cont_fun (String.create size)

  let alloc pool size = String.create size
  ...
  let marshal pool obj flags = Marshal.to_string obj flags
...
let tcp_recv_packet sock hdr ofs len iov =
  ...
  String.blit recv_buf len iov 0 (Iov.len iov);
  got_len
...
(*  log (fun () -> sprintf "sendtosv %s\n" (string_of_info info));*)
  ...
  udp_send info str 0 (String.length str)
...
let udp_mu_recv_packet pool sock mlbuf= 
  try 
    ...
    let len = 
      if is_unix then 
	Unix.recv sock mlbuf 0 max_msg_size [] 
      else
	let len,_ = Unix.recvfrom sock mlbuf 0 max_msg_size [] in 
	len
    in
    if len = max_msg_size then (
      ...
      let hdr_len = read_int32 mlbuf 0 in
      let iovec_len = read_int32 mlbuf 4 in
      if 8+hdr_len+iovec_len <> len then 
	0,empty
      else (
	(*log (fun () -> sprintf "udp_recv_packet, hdr=%d iovl=%d\n" hdr_len iovec_len); *)
	let iovec = String.sub mlbuf (8+hdr_len) (len-hdr_len-8) in
	hdr_len, iovec
      )
