...
value skt_recvfrom(value sock_v, value buf_v, value ofs_v, value len_v)
{
    ...
    SKTTRACE(("skt_recvfrom(skt=%d ofs=%d len=%d  ",
    ...
    )
{
    ...
    sock = Socket_val(sock_v);
    ret = WSASend(sock, send_iova, nvecs, &len, 0, NULL, NULL);
    if (SOCKET_ERROR == ret) {
    ...
    )
{
    ...
    sock = Socket_val(sock_v);
    ret = WSASend(sock, send_iova, nvecs+1, &len, 0, NULL, NULL);
    if (SOCKET_ERROR == ret) {
    ...
    )
{
    ...
    sock = Socket_val(sock_v);
    ret = WSASend(sock, send_iova, nvecs+2, &len, 0, NULL, NULL);
    if (SOCKET_ERROR == ret) {
    ...
    )
{
    ...
    ret = WSARecv(Socket_val(sock_v), recv_iova, 2, &len, &flags , NULL, NULL);
    if (SOCKET_ERROR == ret) {
    ...
    )
{
	...
	if (SOCKET_ERROR == ret) skt_udp_error("skt_udp_send");
    }
    SKTTRACE((")\n"));
    return Val_unit;
}
