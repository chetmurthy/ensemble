...
(* create a UDP socket listening on the specific port. 
*)
let create_sock port = 
  (* Create datagram socket.
   *)
  let sock = Hsys.udp_socket 30000 in
 ...
 *)
let intf alarm port initial_sock = 
  ...
  (* Setting the initial socket
   *)
  let sock = ref initial_sock  in
  Alarm.add_sock_recv alarm name !sock (Hsys.Handler0 recv_gossip) ;

  (* If an error occurs then close the socket, and open a new one. 
   *)
  let switch_socks () = 
    Alarm.rmv_sock_recv alarm !sock ;
    Hsys.close !sock;
    let new_sock = create_sock port in
    sock := new_sock;
    Alarm.add_sock_recv alarm name !sock (Hsys.Handler0 recv_gossip)
  in
  ...
  let install (ls,vs) =
	...
	with Unix.Unix_error(err,s1,s2) -> 
	  match err with 
	    ...
	    | Unix.EBADF -> ()
	    | _ -> 
		printf "REFLECT: A socket error has occured (%s,%s,%s)\n" 
		(Unix.error_message err) s1 s2;
		printf "restarting gossip server\n";
		switch_socks ()
    in
...
let init alarm (ls,vs) port force =
   ...
   *)
  let interface = intf alarm port sock in
