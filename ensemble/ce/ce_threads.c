...
ce_thread_Create(void (*routine)(void*), void *arg, int stacksize)
{
	...
	ce_panic("ce_thread_Create: t_create");
    TRACE(")\n");
}
...
ce_sema_Create(int initial_value) 
{
    ...
    if (sp == NULL)
	ce_panic("ce_sema_Create:  malloc returned NULL");
    
    if ((sp->sema = CreateSemaphore(NULL, initial_value, 0x7fffffff, NULL)) == NULL) {
        printf("gle %d\n",GetLastError());
        ce_panic("ce_sema_Create: failed to create semaphore");
    }
 ...
 */
	...
	ce_panic("ce_thread_Create: pthread_create");
...
ce_sema_Create(int initial_value)
{
	...
	ce_panic("ce_sema_Create: sema_init") ;
    return sp;
		 ...
		 int stacksize) 
{
    if (thr_create(NULL, stacksize, 
		   (ce_thread_routine) routine, arg, 0, NULL))     
		 ...
		 int stacksize)
{
  ...
  if (attr_stat == -1 )
    ce_panic("ce_thread_Create: pthread_attr_create");
...
ce_sema_Destroy(ce_sema_t *sp) {
    ...
    if (sem_destroy(&sp->sema))
	ce_panic("ce_sema_Destroy: sem_destroy");
...
ce_lck_Create(void)
{
	...
	ce_panic("ce_lck_Create: pthread_mutex_init") ;
    return l;
}
/*endproc ce_lck_Create()*/
